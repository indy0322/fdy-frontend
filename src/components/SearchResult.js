...
const SearchResult = (props) => {
    ...
    useEffect(() => {
        const fetchFlowerInfo = async () => {
            try{
                const response = await axios.get(`https://fdy-backend-server-jeqf.onrender.com/flowers?flowername=${label}`);
                setFlowerInfo(response.data)
            }catch(error){
                console.log('Error searching for flower: ',error)
                setFlowerInfo(null)
            }
        }
        fetchFlowerInfo();
                    ...
                    <table style={{borderSpacing: '10px', wordBreak: 'keep-all'}}>
                        <tbody>
                            <tr>
                                ...
                                <td style={{color:'black', width:'85%'}}>{flowerInfo.flowername_kr}</td>
                            </tr>
                            <tr>
