...
const Main = () => {
    ...
    const [flowerKr, setFlowerKr] = useState('')
    ...
    */
    ...
    /*const flowerKR =async (key) => {
        const response = await axios.get(`https://fdy-backend-server-jeqf.onrender.com/flowers?flowername=${key}`);
        console.log(response.data)
        return response.data.flowername_kr
    }*/
    ...
    const detectFrame = async (source, model, canvasRef, callback = () => { }) => {
        ...
        */
        ...
        for (let i = 0; i < detReady.shape[0]; i++) {
            ...
            const response = await axios.get(`https://fdy-backend-server-jeqf.onrender.com/flowers?flowername=${labels[label]}`);
            kr = response.data.flowername_kr
            toDraw.push({
                box: upSampleBox,
                score: score,
                class: label,
                //label: labels[label],
                label: response.data.flowername_kr,
                color: color,
